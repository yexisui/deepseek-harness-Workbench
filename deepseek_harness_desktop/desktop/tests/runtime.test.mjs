...
test('external development is opt-in, ignored when packaged, and requires absolute paths', () => {
  assert.equal(resolveDevelopmentRuntime({}), undefined);
  assert.equal(resolveDevelopmentRuntime({ DSH_DESKTOP_DEV_ROOT: 'bad' }, true), undefined);
  assert.throws(() => resolveDevelopmentRuntime({ DSH_DESKTOP_DEV_ROOT: 'relative' }), /absolute/);
  const root = path.resolve(os.tmpdir(), 'workbench with spaces');
  assert.throws(() => resolveDevelopmentRuntime({ DSH_DESKTOP_DEV_ROOT: root }), /absolute/);
  const runtime = resolveDevelopmentRuntime({ DSH_DESKTOP_DEV_ROOT: root, DSH_DESKTOP_NODE: process.execPath }, false, 'win32');
  assert.equal(runtime.hostRoot, path.join(root, 'runtime'));
  assert.equal(runtime.nodeHome, path.dirname(process.execPath));
  assert.equal(runtime.nodeBin, process.execPath);
  assert.equal(runtime.stateRoot, path.join(root, 'desktop-state'));
  assert.ok(runtime.hostBin.endsWith(path.join('@deepseek-ai', 'dsh', 'lib', 'bin.js')));
  assert.equal(runtime.profileSeed, undefined);
});

test('external development requires a profile and leaves its contents unchanged', () => {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'dsh-external-profile-'));
  try {
    assert.throws(() => requireDevelopmentProfile(home), /Initialize the web profile/);
    const profile = path.join(home, 'profiles', 'web');
    fs.mkdirSync(profile, { recursive: true });
    const manifest = path.join(profile, 'package.json');
    fs.writeFileSync(manifest, '{"name":"user-managed"}');
    requireDevelopmentProfile(home);
    assert.equal(fs.readFileSync(manifest, 'utf8'), '{"name":"user-managed"}');
    assert.equal(fs.existsSync(path.join(profile, SEED_MARKER)), false);
  } finally {
