...
import { detectOfficialChannels, dshSpawnCommand, findDshBinary, quoteForwardedLocalSpecs, unsafeSpecReason, windowsCmdShimArgs } from '../src/host/gateway.ts'
...
describe('desktop .cmd execution', () => {
  it('preserves a spaced local package spec through the official CLI second shell', () => {
    const spec = 'file:C:\\工作台 资料\\workshop\\demo'
    expect(quoteForwardedLocalSpecs(['plugin', 'add', spec], 'win32')).toEqual(['plugin', 'add', `"${spec}"`])
    expect(quoteForwardedLocalSpecs(['add', 'file:./demo'], 'win32', 'C:\\工作台 资料')).toEqual(['add', '"file:C:\\工作台 资料\\demo"'])
    expect(quoteForwardedLocalSpecs(['add', 'pkg@1.0'], 'win32')).toEqual(['add', 'pkg@1.0'])
    expect(quoteForwardedLocalSpecs(['add', 'file:/a b'], 'linux')).toEqual(['add', 'file:/a b'])
    expect(() => quoteForwardedLocalSpecs(['file:C:\\a b\\%PATH%'], 'win32')).toThrow(/unsafe/)
    expect(() => quoteForwardedLocalSpecs(['file:C:\\a b\\"bad'], 'win32')).toThrow(/unsafe/)
  })
