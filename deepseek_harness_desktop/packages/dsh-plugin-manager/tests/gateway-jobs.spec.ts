...
describe('removed plugin updates', () => {
  it('rejects update and migration before spawning or changing the profile', async () => {
    const { facts, dir } = makeProfile({ 'dsh-memoir': {} })
    tempDirs.push(dir)
    const calls: string[][] = []
    const gateway = gatewayFor(facts, () => ({ code: 0 }), calls)
    const before = readFileSync(facts.packageJsonPath, 'utf8')
    for (const result of [gateway.update('dsh-memoir', '1.1.0'), gateway.migrate('old', 'new', '1.0.0', 'new@1.0.0')]) {
      const job = gateway.status(result.jobId)
      expect(job?.phase).toBe('error')
      expect(job?.error).toContain('updates are disabled')
    }
    expect(calls).toEqual([])
    expect(readFileSync(facts.packageJsonPath, 'utf8')).toBe(before)
  })
})
