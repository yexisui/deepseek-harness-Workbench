...
describe('removed plugin update service', () => {
  it('rejects both update operations without selecting a mode, fetching, or calling official RPC', async () => {
    const fetch = vi.fn()
    vi.stubGlobal('fetch', fetch)
    const { face, rpcCall } = setup()
    const changed = vi.fn()
    face.onChange(changed)
    await expect(face.checkUpdates()).rejects.toThrow('updates are disabled')
    await expect(face.update('p1')).rejects.toThrow('updates are disabled')
