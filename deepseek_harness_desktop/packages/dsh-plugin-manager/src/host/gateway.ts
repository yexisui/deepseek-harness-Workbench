...
/**
 * The official Windows plugin CLI forwards its argv through a second cmd.exe
 * shell when starting pnpm. Carry literal quotes through the shell-free Node
 * invocation so a trusted local path containing spaces remains one pnpm spec.
 * Callers supply raw specs; quotes are introduced only after validation here.
 */
export function quoteForwardedLocalSpecs(args: readonly string[], platform: string = process.platform, cwd: string = process.cwd()): string[] {
  if (platform !== 'win32') return [...args]
  return args.map(argument => {
    const match = /^(file:|link:)(.+)$/.exec(argument)
    if (!match) return argument
    if (unsafeSpecReason(argument)) throw new Error('plugin-manager: unsafe local package path')
    const absolute = win32.isAbsolute(match[2]) ? match[2] : win32.resolve(cwd, match[2])
    const spec = match[1] + absolute
...
export function spawnDsh(binary: string, args: string[], env: NodeJS.ProcessEnv) {
  let { command, argsPrefix } = dshSpawnCommand(binary)
  const forwarded = quoteForwardedLocalSpecs(args)
  if (process.platform === 'win32' && command.toLowerCase().endsWith('.cmd') && forwarded.some((argument, i) => argument !== args[i])) {
    const root = env.DSH_DESKTOP_DEV_ROOT
    if (root) ({ command, argsPrefix } = dshSpawnCommand(win32.join(root, 'runtime', 'node_modules', '.bin', 'dsh.cmd')))
    if (command.toLowerCase().endsWith('.cmd')) {
      throw new Error('plugin-manager: local package paths with spaces require the official Node CLI; add its node_modules/.bin directory to PATH')
    }
  }
  ...
  return spawn(command, [...argsPrefix, ...forwarded], {
...
export class CliGateway {
  ...
  /** Compatibility rejection: plugin update/migration never starts a CLI job. */
  update(id: string, version: string): { jobId: string } {
    return this.rejectUpdate('update', id + '@' + version)
  }

  migrate(_sourceId: string, _targetId: string, _targetVersion: string, targetSpec: string): { jobId: string } {
    return this.rejectUpdate('migrate', targetSpec)
  }

  private rejectUpdate(action: 'update' | 'migrate', spec: string): { jobId: string } {
    const job: GatewayJob = { id: 'job-' + ++this.counter, action, spec, phase: 'error', error: 'plugin-manager: plugin updates are disabled in this deployment' }
    this.jobs.set(job.id, job)
    this.retainFinished(job.id)
    return { jobId: job.id }
  }
