...
export interface PluginManagerTabInjected {
  ...
  /** Compatibility method: rejects because plugin updates are disabled. */
  ...
  /** Compatibility method: rejects without contacting registries. */
  checkUpdates: () => Promise<PluginUpdateItem[]>
  /** Read the current install progress. */
...
type BusyAction = { readonly kind: 'install' | 'uninstall'; readonly id?: string }
...
export function PluginManagerTab(props: PluginManagerTabProps) {
  ...
  useEffect(() => {
    if (busy === undefined || busy.kind !== 'install') {
