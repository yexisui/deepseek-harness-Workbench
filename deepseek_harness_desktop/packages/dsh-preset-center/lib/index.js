...
function readProvenance(dir, id) {
	const local = readLocalTrustRecord(dir, "preset", id);
	if (local) return {
		version: 1,
		source: local.source,
		id,
		installedAt: local.importedAt,
		assetVersion: local.versionLabel,
		files: local.files
	};
	let raw;
	try {
...
function verifyProvenance(dir, id) {
	...
	const extra = listFiles(dir).filter((rel) => rel !== "dsh-market.provenance.json" && rel !== "dsh-workbench.import.json" && provenance.files[rel] === void 0);
	const local = readLocalTrustRecord(dir, "preset", id);
	return {
		state: mismatches.length === 0 && missing.length === 0 && (!local || localResourceUnchanged(dir, local)) ? "valid" : "modified",
		provenance,
		mismatches,
		missing,
		extra
...
function enablePreset(dshHome, id) {
	...
	if (readLocalTrustRecord(src, "preset", id) && !isLocalResourceTrusted(dshHome, src, "preset", id)) throw new PresetOperationError("confirmation-required", "Local preset content changed or has not been confirmed");
...
function makePresetCenterRoutes(deps = {}) {
	...
	const handleEnable = guardedHandler("preset-center/enable", async (req, res) => {
		...
		const local = readLocalTrustRecord(state.dir, "preset", id);
		if (local && !localResourceUnchanged(state.dir, local)) {
			sendError(res, ERRORS["broken"], "Local preset files changed; import and confirm the updated resource again");
			return;
		}
		if ((needsConfirmation(profile) || local && !isLocalResourceTrusted(home, state.dir, "preset", id)) && !confirm) {
			send(res, 409, {
				ok: false,
				error: "confirmation-required",
		...
		if (local && confirm) trustLocalResource(home, state.dir, local);
		try {
...
function registerWorkshopServiceRoutes(home, routes) {
	const entries = routes.filter((route) => route.kind === "exact");
	const address = (path) => `${resolve(home)}:${path}`;
	for (const route of entries) registry().set(address(route.path), route);
	return () => {
		for (const route of entries) if (registry().get(address(route.path)) === route) registry().delete(address(route.path));
	};
}
//#endregion
//#region src/index.ts
...
function applyImpl(ctx) {
	...
	ctx.effect(() => registerWorkshopServiceRoutes(dshHome(), routes), "dsh-preset-center: workshop lifecycle");
