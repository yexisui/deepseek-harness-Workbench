...
import { isLocalResourceTrusted, readLocalTrustRecord } from '../../../../shared/host/local-resource-trust.ts'
...
export function enablePreset(dshHome: string, id: string): PresetStateRow {
  ...
  if (readLocalTrustRecord(src, 'preset', id) && !isLocalResourceTrusted(dshHome, src, 'preset', id)) {
    throw new PresetOperationError('confirmation-required', 'Local preset content changed or has not been confirmed')
  }
