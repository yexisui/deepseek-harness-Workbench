...
import { LOCAL_IMPORT_RECORD, localResourceUnchanged, readLocalTrustRecord } from '../../../../shared/host/local-resource-trust.ts'
...
export function readProvenance(dir: string, id: string): PresetProvenance | null {
  ...
  if (local) return { version: 1, source: local.source, id, installedAt: local.importedAt, assetVersion: local.versionLabel, files: local.files }
  let raw: unknown
  try {
...
export function verifyProvenance(dir: string, id: string): ProvenanceReport {
  ...
  const extra = listFiles(dir).filter((rel) => rel !== PROVENANCE_FILENAME && rel !== LOCAL_IMPORT_RECORD && provenance.files[rel] === undefined)
  const local = readLocalTrustRecord(dir, 'preset', id)
  const state: ProvenanceState = mismatches.length === 0 && missing.length === 0 && (!local || localResourceUnchanged(dir, local)) ? 'valid' : 'modified'
