...
export const Config: z<Config> = z.object({ enabled: z.boolean().default(true), fullProtection: z.boolean().default(true), autoRepair: z.boolean().default(false), autoMigrate: z.boolean().default(false), heartbeatIntervalMs: z.number().min(1000).default(5000) })
...
export function effectiveConfig(config?: Config): Required<Config> {
  return { enabled: config?.enabled ?? true, fullProtection: config?.fullProtection ?? DEFAULT_DOCTOR_POLICY.fullProtection, autoRepair: config?.autoRepair ?? DEFAULT_DOCTOR_POLICY.autoRepair, autoMigrate: false, heartbeatIntervalMs: config?.heartbeatIntervalMs ?? 5000 }
}
