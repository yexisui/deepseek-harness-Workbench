...
async function renderSkinCenter(options: {
  runtime?: SkinCenterComponentProps['runtime']
  refreshedCatalog?: () => Promise<typeof mint[]>
  active?: string | null
...
} = {}): Promise<SkinCenterComponentProps['runtime']> {
  ...
  const runtime = options.runtime ?? {
    ...
    refreshCatalog: async () => {
      if (options.refreshedCatalog) catalog = await options.refreshedCatalog()
  ...
  return runtime as SkinCenterComponentProps['runtime']
}
...
describe('SkinCenter catalog after Workshop imports', () => {
  it('refreshes the retained runtime on each opening without changing the active skin', async () => {
    let available = [mint]
    const refreshedCatalog = vi.fn(async () => available)
    const switchTo = vi.fn(async (id: string | null) => id)
    const runtime = await renderSkinCenter({ active: 'mint', refreshedCatalog, switchTo })
    expect(refreshedCatalog).toHaveBeenCalledTimes(1)
    expect(host.textContent).not.toContain('Imported skin')

    await act(async () => { root.render(null) })
    available = [mint, { ...mint, origin: 'user', manifest: { ...mint.manifest, id: 'imported', name: 'Imported skin', nameEn: 'Imported skin' } }]
    await renderSkinCenter({ runtime })

    expect(refreshedCatalog).toHaveBeenCalledTimes(2)
    expect(cardNamed('Imported skin')).toBeTruthy()
    expect(runtime.controller.getState().active).toBe('mint')
    expect(switchTo).not.toHaveBeenCalled()
  })

  it('keeps the previous catalog and active skin when refreshing fails', async () => {
    const switchTo = vi.fn(async (id: string | null) => id)
    const runtime = await renderSkinCenter({ active: 'mint', switchTo, refreshedCatalog: async () => { throw new Error('Host unavailable') } })
    expect(cardNamed('Mint')).toBeTruthy()
    expect(runtime.controller.getState().active).toBe('mint')
    expect(switchTo).not.toHaveBeenCalled()
  })
})
