...
describe('repairSkin and verifyAndRepairAllSkins', () => {
  it('automatically repairs tampered skins from a local source mirror without network access', async () => {
  ...
  it('handles repair failure gracefully when skin is not available', async () => {
    ...
    const fetch = vi.fn(() => { throw new Error('unexpected network request') })
    vi.stubGlobal('fetch', fetch)
    const before = readFileSync(join(user, 'custom-offline', 'skin.json'), 'utf8')
    ...
    expect(summary.repairFailed[0].error).toContain('import this skin again from a local ZIP or folder')
    expect(fetch).not.toHaveBeenCalled()
    expect(readFileSync(join(user, 'custom-offline', 'skin.json'), 'utf8')).toBe(before)
    expect(summary.issues).toBe(1)
  })
})
