...
export function SkinCenter({ t, runtime, theme, background, wallpaper, preview, customTheme }: SkinCenterComponentProps) {
  ...
  useEffect(() => {
    // The runtime survives settings navigation. Refresh only its catalog so
    // resources imported in the Workshop appear when this page is reopened;
    // the active skin and any ongoing preview stay owned by the controller.
    void runtime.refreshCatalog().catch(() => {
      // Keep the last usable catalog on a transient host error. Reopening
      // this page retries without resetting the currently applied skin.
    })
  }, [runtime])
