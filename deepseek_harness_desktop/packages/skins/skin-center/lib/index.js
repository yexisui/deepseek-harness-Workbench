...
function verifyLocalSkinHooks(dir, skinId) {
...
function verifySkinIntegrity(dir, skinId, options = {}) {
	...
	if (local) {
		...
		return {
			id: skinId,
			status: intact ? "valid" : "tampered",
			hooksTrusted: intact && verifyLocalSkinHooks(dir, skinId),
			hasProvenance: true,
			mismatches: intact ? [] : ["local-resource-content"],
			missing: [],
			totalFilesChecked: Object.keys(local.files).length
		};
	}
	let raw = null;
	try {
...
async function repairSkinFromMarket(destDir, skinId, options = {}) {
	...
	return {
		ok: false,
		id: skinId,
		error: "local-source-unavailable: import this skin again from a local ZIP or folder in Workshop"
	};
}
//#endregion
...
function marketHooksTrust(manifest, dir) {
	...
	if (verifyLocalSkinHooks(dir, manifest.id) || verifyMarketProvenance(dir, manifest.id, facet.entry) || verifyReviewedLegacyHooks(dir, manifest.id, facet.entry)) return {
		trusted: true,
		warning: null
	};
	return {
		trusted: false,
		warning: "declares hooks.mjs, but hooks require a built-in skin, verified market provenance, or explicit local Workshop script consent; the hooks facet will be refused"
	};
}
...
async function repairSkin(skinId, options = {}) {
	const res = await repairSkinFromMarket(join(options.userDir ?? userSkinsDir(), skinId), skinId, { localSourceDir: options.localSourceDir });
