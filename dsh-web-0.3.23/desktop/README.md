...
An unpackaged app can use an existing workbench instead of a staged payload: set `DSH_DESKTOP_DEV_ROOT` to its absolute directory and `DSH_DESKTOP_NODE` to the absolute system Node executable. Install the host under that directory's `runtime/node_modules` and explicitly set `DSH_HOME` to an initialized data directory. Launch the desktop source directory with an externally installed Electron. This mode requires an existing web profile and never seeds or reseeds it. Electron user data, browser session data and logs live under `desktop-state/` in the workbench. Packaged apps ignore these development overrides and retain their bundled runtime behavior.

The overrides select runtime paths; they do not build or link local feature plugins. The selected web profile still controls which plugin versions are loaded.
