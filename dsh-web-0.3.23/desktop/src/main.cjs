...
const developmentRuntime = resolveDevelopmentRuntime(process.env, app.isPackaged);
if (developmentRuntime) {
  // Set paths before the single-instance lock and Chromium session initialize.
  for (const [key, dir] of [['userData', 'user-data'], ['sessionData', 'session-data'], ['logs', 'logs']]) {
    const location = path.join(developmentRuntime.stateRoot, dir);
    fs.mkdirSync(location, { recursive: true });
    app.setPath(key, location);
  }
}
...
function startHost(runtime, home, port) {
  const nodePaths = [
    path.join(runtime.hostRoot, 'node_modules'),
...
async function boot() {
  const runtime = developmentRuntime ?? resolveRuntimePaths(resourcesRoot(), process.platform, process.arch, app.isPackaged);
  ...
  if (!fs.existsSync(runtime.nodeBin)) throw new Error('Node runtime is missing: ' + runtime.nodeBin);
  if (!fs.existsSync(runtime.hostBin)) throw new Error('dsh host is missing: ' + runtime.hostBin);

  if (runtime.external) {
    requireDevelopmentProfile(home);
    pushLogLine('[desktop] using external development runtime and existing web profile');
  } else {
  ...
  try {
    ensureProfileFallbacks(home, runtime.hostRoot);
  } catch (error) {
  ...
  pushLogLine('[desktop] GUI loaded');
}
