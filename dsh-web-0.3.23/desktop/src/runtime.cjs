...
function resolveRuntimePaths(resourcesRoot, platform, arch, packaged = true) {
  ...
  return {
    runtimeRoot,
    hostRoot: path.join(runtimeRoot, 'host'),
...
/** Resolve an explicitly selected external development environment. */
function resolveDevelopmentRuntime(env = process.env, packaged = false, platform = process.platform) {
  if (packaged || !env.DSH_DESKTOP_DEV_ROOT) return undefined;
  const root = env.DSH_DESKTOP_DEV_ROOT;
  const nodeBin = env.DSH_DESKTOP_NODE;
  if (!path.isAbsolute(root) || !nodeBin || !path.isAbsolute(nodeBin)) {
    throw new Error('Desktop development requires absolute DSH_DESKTOP_DEV_ROOT and DSH_DESKTOP_NODE paths.');
  }
  const hostRoot = path.join(root, 'runtime');
  return {
    external: true,
    runtimeRoot: hostRoot,
    hostRoot,
    nodeBin,
    nodeHome: platform === 'win32' ? path.dirname(nodeBin) : path.dirname(path.dirname(nodeBin)),
    hostBin: path.join(hostRoot, 'node_modules', '@deepseek-ai', 'dsh', 'lib', 'bin.js'),
    stateRoot: path.join(root, 'desktop-state'),
  };
}

/** External development uses an existing profile and never seeds over it. */
function requireDevelopmentProfile(home) {
  const manifestPath = path.join(home, 'profiles', 'web', 'package.json');
  if (!fs.existsSync(manifestPath)) {
    throw new Error('Initialize the web profile in DSH_HOME before starting desktop development: ' + manifestPath);
  }
}
