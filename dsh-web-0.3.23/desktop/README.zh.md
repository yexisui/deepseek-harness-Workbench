...
未打包应用可以使用已有工作台代替暂存载荷：将 `DSH_DESKTOP_DEV_ROOT` 设为工作台绝对目录，将 `DSH_DESKTOP_NODE` 设为系统 Node 可执行文件的绝对路径。宿主安装在该目录的 `runtime/node_modules` 下，并显式将 `DSH_HOME` 指向已初始化的数据目录。使用独立安装的 Electron 启动桌面源码目录。此模式要求已有 web profile，绝不初始化或重新播种该 profile。Electron 用户数据、浏览器会话数据与日志放在工作台的 `desktop-state/` 下。打包后的应用忽略这些开发覆盖参数，仍使用内置运行时。

这些参数只选择运行路径，不构建或链接本地功能插件；具体加载哪个版本的插件，仍由选定的 web profile 决定。
