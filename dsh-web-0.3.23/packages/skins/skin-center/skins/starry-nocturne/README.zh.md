...
星夜夜曲（Starry Nocturne）—— dsh web GUI 的哥特蕾丝星空皮肤，以纯资产目录
...
- **背景**：蕾丝白画布上的星夜裙立绘（亮色），暗色化为整幅星空少女场景铺底、
  银尘星屑漂浮，通过 `contributes.backgroundMedia` 声明（亮/暗遮罩），由
  皮肤中心背景控件拥有；遮罩随亮/暗主题实时切换。
- **token 优先**：亮色值挂在 `:root`（蕾丝白、丝绒午夜强调），暗色值挂在
  `body[data-ds-dark-theme]`（丝绒午夜、紫水晶星光）；加载器把每条选择器
  作用域到 `html[data-dsh-skin="starry-nocturne"]`。皇室蓝紫 `#5B5FC7`
  贯穿令牌层，一缕红丝带微光作为秘密记号。
- **Hooks**：`hooks.mjs` 通过皮肤中心 hooks 契约
  （`x-org.linxin666.skin-center/v1alpha1`）渲染 16 颗漂浮银尘：纯装饰、
