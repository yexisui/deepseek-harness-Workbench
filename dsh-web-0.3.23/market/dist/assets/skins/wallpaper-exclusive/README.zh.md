...
表面——消息气泡、代码块、行内代码、设置卡片、插件面板、技能中心与任务清单——
统一渲染为液态玻璃，让 Wallpaper Engine 的壁纸透出；输入卡本身由 web-ui 统一设计。
皮肤内置默认壁纸 `whale-art-v2.png`，仅在未选择 Wallpaper Engine 壁纸时使用；
否则 WE 壁纸即背景。

## 功能

- 壁纸优先：WE 壁纸即背景，其上不再画任何不透明层。
- 全表面液态玻璃：用户气泡、代码块、行内代码、设置表面、插件面板、侧栏按键
